...
export const InvoiceRow: React.FC<InvoiceRowProps> = ({ invoice }) => {
  return (
    <tr className="border-b border-dark-border hover:bg-dark-elevated/30 transition-all">
      ...
      <td className="py-4 px-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">{invoice.dentist.clinic}</p>
          </div>
        </div>
      </td>
      <td className="py-4 px-4">
      ...
      <td className="py-4 px-4">
        <span className="text-sm text-slate-800 font-semibold">${invoice.grossAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
      </td>
      <td className="py-4 px-4">
