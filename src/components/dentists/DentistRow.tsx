...
export const DentistRow: React.FC<DentistRowProps> = ({
  ...
  return (
    <>
      ...
      >
        <td className="py-4 px-4">
          <div className="flex items-center gap-2">
            {hasSubDentists && (
              <button className="text-slate-500 hover:text-slate-900 transition-colors mr-1">
                {isExpanded ? (
            ...
            <div className={cn("flex items-center gap-3", isSubItem && "pl-6")}>
              <img src={avatarSrc} alt={dentist.firstName} className="w-10 h-10 rounded-full object-cover border border-dark-border" />
        ...
        <td className="py-4 px-4">
          {isMainDentist ? (
            <div>
              ...
              <p className="text-xs text-slate-500">{mainDentist.accountStatus || 'Active'}</p>
            </div>
          ) : (
            <div>
              <p className="text-slate-800 font-medium">{(dentist as SubDentist).locationName || 'Branch'}</p>
              <p className="text-xs text-slate-500">{(dentist as SubDentist).locationAddress || '--'}</p>
            </div>
          )}
        </td>
        <td className="py-4 px-4">
      ...
      {/* Sub-dentists */}
      {isExpanded && isMainDentist && mainDentist.subDentists?.map((sub) => (
        <DentistRow key={sub.userId} dentist={sub} isSubItem />
      ))}
    </>
  );
};
